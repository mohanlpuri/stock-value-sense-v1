...
const YELLOW = { red: 1.00, green: 0.95, blue: 0.70 }
const NONE   = { red: 1.00, green: 1.00, blue: 1.00 }
...
async function saveToSheet(token, sheetId, stock) {
  ...
  const existingIdx = existing.findIndex(function(r, i) { return i > 0 && r[1] === stock.ticker })
...
async function fetchStock(ticker) {
  ...
  const evEbitdaRaw = metrics['enterpriseValueEbitdaTTM'] || metrics['evEbitdaTTM'] || metrics['evEBITDA'] || null
  const evEbitda    = evEbitdaRaw != null ? parseFloat(evEbitdaRaw.toFixed(2)) : null

  var analystRating = null
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (body.action === 'saveRatings') {
      ...
      const existing    = await readSheet(token)
      const existingIdx = existing.findIndex(function(r, i) { return i > 0 && r[1] === ticker })
      ...
      const putUrl = 'https://sheets.googleapis.com/v4/spreadsheets/' + SHEET_ID + '/values/Sheet1!A' + rowNum + ':O' + rowNum + '?valueInputOption=RAW'
      await fetch(putUrl, {
        method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
